...
func RunServeAdminForCorp104(c *config.Config) func(cmd *cobra.Command, args []string) {
	return func(cmd *cobra.Command, args []string) {
		c.MustValidate()
		checkDatabaseAllowed(c)
		serverHandler, _, backend, mws := setupForCorp104(c, cmd, args, "admin")

		var wg sync.WaitGroup
		wg.Add(2)

		cert := getOrCreateTLSCertificateForCorp104(cmd, c)
		// go serve(c, cmd, enhanceRouter(c, cmd, serverHandler, frontend), c.GetFrontendAddress(), &wg)
		go serve(c, cmd, EnhanceRouter(c, cmd, serverHandler, backend, mws, viper.GetString("CORS_ENABLED") == "true"), c.GetBackendAddress(), &wg, cert)
...
func RunServePublicForCorp104(c *config.Config) func(cmd *cobra.Command, args []string) {
	return func(cmd *cobra.Command, args []string) {
		c.MustValidate()
		checkDatabaseAllowed(c)
		serverHandler, frontend, _, mws := setupForCorp104(c, cmd, args, "public")
...
func RunServeAllForCorp104(c *config.Config) func(cmd *cobra.Command, args []string) {
	return func(cmd *cobra.Command, args []string) {
		c.MustValidate()
		serverHandler, frontend, backend, mws := setupForCorp104(c, cmd, args, "all")
...
func setupForCorp104(c *config.Config, cmd *cobra.Command, args []string, name string) (handler *Handler, frontend, backend *httprouter.Router, middlewares []negroni.Handler) {
	fmt.Println(banner(c.BuildVersion))
	...
	handler = NewHandler(c, w)
	handler.RegisterRoutesForCorp104(frontend, backend)
	c.ForceHTTP, _ = cmd.Flags().GetBool("dangerous-force-http")
...
func (h *Handler) RegisterRoutesForCorp104(frontend, backend *httprouter.Router) {
	c := h.Config
	ctx := c.Context()
	...
	oauth2Provider := newOAuth2ProviderForCorp104(c)

	// Set up handlers
	h.Clients = newClientHandler(c, backend, clientsManager)
	h.Keys = newJWKHandler(c, frontend, backend)
	h.Consent = newConsentHandler(c, frontend, backend)
	h.OAuth2 = newOAuth2HandlerForCorp104(c, frontend, backend, ctx.ConsentManager, oauth2Provider, clientsManager)
	_ = newHealthHandler(c, backend)
}
