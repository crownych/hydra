...
import (
	...
	hc "github.com/ory/hydra/corp104/client"
			...
			for km, fs := range fositeStores {
				t.Run("manager="+km, func(t *testing.T) {
					...
					oauthConfig := &oauth2.Config{
						...
						Endpoint:    oauth2.Endpoint{AuthURL: ts.URL + AuthPath, TokenURL: ts.URL + TokenPath},
		...
		t.Run("strategy="+strat.d, func(t *testing.T) {
			...
			oauthConfig := &oauth2.Config{
				...
				Endpoint: oauth2.Endpoint{
					AuthURL:  ts.URL + AuthPath,
					TokenURL: ts.URL + TokenPath,
				},
			...
			for k, tc := range []struct {
				...
				t.Run(fmt.Sprintf("case=%d/description=%s", k, tc.d), func(t *testing.T) {
					...
					t.Run("case=userinfo", func(t *testing.T) {
						...
						req, err = http.NewRequest("GET", ts.URL+UserinfoPath, nil)
						req.Header.Add("Authorization", "bearer "+token.AccessToken)
						testSuccess(makeRequest(req))

						req, err = http.NewRequest("POST", ts.URL+UserinfoPath, nil)
						req.Header.Add("Authorization", "bearer "+token.AccessToken)
						testSuccess(makeRequest(req))

						req, err = http.NewRequest("POST", ts.URL+UserinfoPath, bytes.NewBuffer([]byte("access_token="+token.AccessToken)))
