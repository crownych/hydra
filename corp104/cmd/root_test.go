...
func TestExecute(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{args: []string{"clients", "create", "--endpoint", frontend, "--id", "foobarbaz", "--name", "foobar", "-g", "urn:ietf:params:oauth:grant-type:token-exchange", "--client-uri", "http://myapp.com", "--contacts", "admin@myapp.com", "--software-id", "4d51529c-37cd-424c-ba19-cba742d60903", "--software-version", "0.0.1", "--token-endpoint-auth-method", "private_key_jwt", "--jwks", `[{"use":"sig","kty":"EC","kid":"public:89b940e8-a16f-48ce-a238-b52d7e252634","crv":"P-256","alg":"ES256","x":"6yi0V0cyxGVc5fEiu2U2PuZr4TxavTguccdcco1XyuA","y":"kX_biw0hYHyt1qaVP4EbP7WScIu9QyPK0Aj3fXpBRCg"}]`, "--signing-jwk", `{"use":"sig","kty":"EC","kid":"private:89b940e8-a16f-48ce-a238-b52d7e252634","crv":"P-256","alg":"ES256","x":"6yi0V0cyxGVc5fEiu2U2PuZr4TxavTguccdcco1XyuA","y":"kX_biw0hYHyt1qaVP4EbP7WScIu9QyPK0Aj3fXpBRCg","d":"G4ExPHksANQZgLJzElHUGL43The7h0AKJE69qrgcZRo"}`}},
		{args: []string{"clients", "get", "--endpoint", frontend, "foobarbaz"}},
		{args: []string{"clients", "create", "--endpoint", frontend, "--id", "public-foo"}},
		{args: []string{"clients", "delete", "--endpoint", frontend, "public-foo"}},
